...
* Additional Query Management Support
    - xref:../../core-functions/pages/query.adoc#_showquery[`showQuery()`] returns the GSQL of a given query.
    - xref:../../core-functions/pages/query.adoc#_getquerymetadata[`getQueryMetadata()`] returns the metadata details about a query, such as input parameters and what is returned in `PRINT` statements.
    - xref:../../core-functions/pages/query.adoc#_getrunningqueries[`getRunningQueries()`] shows the statuistics of queries currently running on the graph.
    - xref:../../core-functions/pages/query.adoc#_abortquery[`abortQuery()`] aborts a selected query by ID or all queries on the graph.
