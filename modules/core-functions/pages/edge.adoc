...
#creating-an-edge-from-or-to-any-vertex-type
 +
See https://docs.tigergraph.com/v/2.6/dev/gsql-ref/ddl-and-loading/defining-a-graph-schema[this] for more details.
...
#_list_edges_of_a_vertex
 +
See https://docs.tigergraph.com/tigergraph-server/current/api/built-in-endpoints[this] for more details.
...
#operation-codes .
For valid values of `<operator>` see https://docs.tigergraph.com/dev/restpp-api/built-in-endpoints[this]
...
be included in the dataframe?
* `withType`: (When the output format is "df") should the edge type be included in the dataframe?
* `limit`: Maximum number of edge instances to be returned. Default is None (no limit).
